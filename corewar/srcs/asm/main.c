...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/02 20:41:42 by lfarlen           #+#    #+#             */
/*   Updated: 2019/12/14 14:49:48 by qhetting         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(int ac, char **av)
{
	...
	if ((instructions_init(&info, info.ins)) == 1)
	{
		*info.file = 1;
		ft_usage(6, &info);
		return (free_for_all(&info));
	}
	while (!(get_instruction_list(&info, info.ins)))
		if (info.error)
			return (free_for_all(&info));
	if (info.error)
		return (free_for_all(&info));
